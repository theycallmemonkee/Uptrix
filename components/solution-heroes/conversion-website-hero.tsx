...
// ── Hardcoded config for Conversion Website System — edit only this file ──
const IMAGE_SRC =
  "https://images.unsplash.com/photo-1507238691740-187a5b1d37b8?w=960&q=85";
const CARD_VALUE = "3.21X";
const CARD_LABEL = "ROAS Delivered";
const CARD_SUBLABEL = "Avg. across client sites";

type Props = { solution: SolutionConfig };

export function ConversionWebsiteHero({ solution }: Props) {
  const IconComp = ICON_MAP[solution.iconName as keyof typeof ICON_MAP] ?? Globe;
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-6 md:px-10 lg:grid-cols-[1fr_1fr] lg:gap-14 xl:gap-16">
        ...
        >
          ...
          <motion.h1
            variants={itemVariants}
            className="font-heading text-[clamp(1.9rem,2.9vw,3.2rem)] leading-[1.12] font-extrabold tracking-[-0.025em] text-white"
          >
            <span className="block">Getting Visitors but</span>
        ...
        >
          ...
          >
            <div className="relative aspect-[4/3] w-full">
              <Image
                src={IMAGE_SRC}
                alt="Website conversion design session"
                fill
