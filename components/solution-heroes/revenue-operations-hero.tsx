...
// ── Hardcoded config for Revenue Operations System — edit only this file ──
const IMAGE_SRC =
  "https://images.unsplash.com/photo-1551818255-e6e10975bc17?w=960&q=85";
const CARD_VALUE = "+52%";
const CARD_LABEL = "Lead Velocity";
const CARD_SUBLABEL = "After RevOps implementation";

type Props = { solution: SolutionConfig };

export function RevenueOperationsHero({ solution }: Props) {
  const IconComp = ICON_MAP[solution.iconName as keyof typeof ICON_MAP] ?? Layers;
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-6 md:px-10 lg:grid-cols-[1fr_1fr] lg:gap-14 xl:gap-16">
        ...
        >
          ...
          <motion.h1
            variants={itemVariants}
            className="font-heading text-[clamp(1.9rem,2.8vw,3.1rem)] leading-[1.12] font-extrabold tracking-[-0.025em] text-white"
          >
            <span className="block">Growing Fast but Everything</span>
        ...
        >
          ...
          >
            <div className="relative aspect-[4/3] w-full">
              <Image
                src={IMAGE_SRC}
                alt="Revenue operations planning session"
                fill
