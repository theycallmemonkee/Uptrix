...
// ── Hardcoded config for Demand Generation System — edit only this file ──
// CTA callbacks allow the parent (MoreLeadsClient) to wire up scroll-to-section.
type Props = {
  onPrimaryClick?: () => void;
  onSecondaryClick?: () => void;
};

export function DemandGenerationHero({ onPrimaryClick, onSecondaryClick }: Props) {
  // Mouse-reactive spring system — matches premium quality of other solution heroes
  ...
  return (
    ...
    >
      {/* Mouse-reactive glow — interactive per-hero layer */}
      ...
      {/* ── Two-column grid: equal split gives text column enough room for 2-line headline ── */}
      <div className="mx-auto grid w-full max-w-7xl items-center gap-10 px-6 md:px-10 lg:grid-cols-[1fr_1fr] lg:gap-14 xl:gap-16">
        ...
        >
          ...
          >
            <Sparkles size={10} className="shrink-0 text-[#79ABFF]" />
            <span>Demand Generation System</span>
          </motion.div>

          {/* Headline — two explicit blocks guarantee 2-line layout on desktop */}
          <motion.h1
            variants={itemVariants}
            className="font-heading text-[clamp(1.8rem,2.6vw,2.8rem)] leading-[1.12] font-extrabold tracking-[-0.025em] text-white"
          >
            <span className="block">Tired of Wondering Where</span>
            <span className="block mt-1">
              Your Next Customer Will{" "}
              <motion.span
          ...
          <motion.div
            variants={itemVariants}
            className="mt-6 space-y-3 max-w-[460px] text-[0.9375rem] leading-[1.78] text-white/60 mx-auto lg:mx-0"
          >
            <p>
              Most businesses run scattered tactics and pray something works. We build one connected engine that brings customers to you on repeat, every single month.
            </p>
            <p className="font-semibold text-white/85">
              Stop depending on hope. Build a lead engine that never runs dry.
            </p>
          </motion.div>
        ...
        >
          ...
          {/* Image card with subtle 3-D tilt */}
          <motion.div
          ...
          >
            <div className="relative aspect-[4/3] w-full">
              ...
              {/* Subtle blue screen tint for premium tone */}
          ...
          {/* Single floating metric card — bottom-left */}
          <motion.div
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[7.5px] font-semibold tracking-[0.14em] text-white/40 uppercase">Qualified Leads</span>
                <span className="flex h-3.5 w-3.5 items-center justify-center rounded bg-emerald-500/10 text-emerald-400 text-[9px]">↑</span>
              </div>
              <p className="font-heading text-[1.5rem] font-bold leading-none text-white">+240%</p>
              <p className="text-[7px] text-white/44 leading-relaxed mt-1">Pipeline growth in 90 days</p>
