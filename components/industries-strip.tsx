...
export interface IndustriesStripProps {
  data?: SanityHomePage | null
}

export function IndustriesStrip({ data }: IndustriesStripProps = {}) {
  const eyebrow = data?.industriesEyebrow ?? "WHO WE WORK WITH";
  const headline = data?.industriesHeadline ?? "Growth Systems for Every Kind of Business";
  const body = data?.industriesBody ?? "The systems adapt to your business. The goal stays the same. More customers, more revenue, less chaos.";
